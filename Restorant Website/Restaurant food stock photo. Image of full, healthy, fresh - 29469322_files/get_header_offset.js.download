...
const getOffsetElement=(offset)=>{if(!DT.isNullish(offset.element))return offset.element;const element=document.querySelector(offset.selector);if(!element){DT.log('error',`Element not found for selector: ${offset.selector}`)}
